      ...
      `}>
        ...
        <nav className="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto scrollbar-hide">
          {menuItems.map((item) => (
            ...
            >
              <item.icon className={`shrink-0 transition-all duration-200 ${isOpen ? 'w-5 h-5 mr-3' : 'w-6 h-6'
                }`} />
        ...
        {/* Footer */}
        <div className={`p-4 border-t border-gray-200 transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'
          }`}>
          {isOpen && (
